...
int mx_send_pubkey(void *arg) {
    ...
    if (sent == -1) {
        ...
        syslog(LOG_INFO, "Sent public key to client ID: %ld", client->thread_id);
    }
...
int mx_recieve_aes(void *arg) {
    ...
    if (received <= 0) {
        syslog(LOG_ERR, "Failed to receive AES key from Client id: %ld", client->thread_id);
        return 1;
    }
    buffer[received] = '\0';
    ...
    if (!aes_key_hex_json || !iv_hex_json) {
        syslog(LOG_ERR, "Missing AES key or IV in JSON from Client id: %ld", client->thread_id);
    ...
    if (aes_key_len == -1) {
        syslog(LOG_ERR, "Failed to decode hex AES key from Client id: %ld", client->thread_id);
        cJSON_Delete(json_payload);
        return 1;
    }
    ...
    if (decrypt_aes_key_with_private_key(encrypted_aes_key, aes_key_len, aes_key) <= 0) {
        syslog(LOG_ERR, "Failed to decrypt AES key from Client id: %ld", client->thread_id);
        free(aes_key);
        cJSON_Delete(json_payload);
        return 1;
    }
    ...
    if (iv_len <= 0) {
        syslog(LOG_ERR, "Failed to decode hex IV from Client id: %ld", client->thread_id);
        free(aes_key);
        cJSON_Delete(json_payload);
        return 1;
    }
    ...
    syslog(LOG_INFO, "Success: AES Key and IV successfully stored for Client id: %ld", client->thread_id);
